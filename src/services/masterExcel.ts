...
import { normalizeExcelDateValue } from '../utils/dateUtils';
...
function dateToExcelDate(value: string): Date {
...
export async function applyMasterChanges(
  ...
  for (const candidate of candidates) {
    ...
    for (const change of candidate.changes) {
      row.getCell(change.masterColumnIndex).value = change.current ? dateToExcelDate(change.current) : null;
    }
  }
