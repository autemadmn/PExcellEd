...
function normalizeDateFromSerial(serial: number): string | null {
  if (!Number.isFinite(serial) || serial <= 0 || serial > 2958465) {
    return null;
  }

  const wholeDays = Math.floor(serial);
  const utcTime = (wholeDays - EXCEL_EPOCH_OFFSET) * MS_PER_DAY;
...
export function normalizeExcelDateValue(value: unknown, displayText?: string): string | null {
  ...
  if (unwrapped === null || unwrapped === undefined || unwrapped === '') {
    return normalizeDateFromString(displayText ?? '');
  }

  if (unwrapped instanceof Date && !Number.isNaN(unwrapped.getTime())) {
    return ymdFromParts(unwrapped.getFullYear(), unwrapped.getMonth() + 1, unwrapped.getDate());
  }

  if (typeof unwrapped === 'number') {
    return normalizeDateFromSerial(unwrapped);
  }

  if (typeof unwrapped === 'string') {
    return normalizeDateFromString(unwrapped) ?? normalizeDateFromString(displayText ?? '');
  }
